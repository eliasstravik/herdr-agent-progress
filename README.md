...
SQLite transactions serialize task generation changes and publication decisions. Publication sequence numbers are persisted before sends, so a timeout or crash cannot reuse a sequence. The publisher uses one stable metadata source, `agent-progress`, and explicitly clears its three tokens. It does not use TTL to implement stale reports.
